...
class UpdateServiceProofListener implements ShouldQueue
{
    use InteractsWithQueue;

    public $queue = 'UpdateServiceProofEvent';

    /**
     * Handle the event.
     */
    public function handle(UpdateServiceProofEvent $event)
    {
        try {
            $booking = $event->booking;
            if (!$booking->provider_id && method_exists($booking, 'sub_bookings')) {
                $subBooking = $booking->sub_bookings()->whereNotNull('provider_id')->first();
                if ($subBooking) {
                    $booking = $subBooking;
                }
            }
    ...
    public function sendPushNotification($topic, $event, $role)
    {
        if (!$topic) return;
            ...
            switch ($role) {
                case RoleEnum::ADMIN:
                    $slug = 'update-service-proof-booking-admin';
                    break;
                case RoleEnum::PROVIDER:
                    $slug = 'update-service-proof-booking-provider';
                    break;
                default:
                    return; 
            }
